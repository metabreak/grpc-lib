...
class GrpcWebClient {
    ...
    unary(path, req, metadata, reqclss, resclss) {
        ...
        return new rxjs_1.Observable((obs) => {
            const stream = this.client.rpcCall(this.settings.host + path, req, metadata?.toObject() ?? {}, descriptor, (error, data) => {
                if (error) {
                    obs.next(new grpc_common_1.GrpcStatusEvent(error.code, error.message, new grpc_common_1.GrpcMetadata(error.metadata)));
                    obs.complete();
                }
                else {
                    obs.next(new grpc_common_1.GrpcDataEvent(data));
                }
            });
            stream.on('status', (status) => status.code === 0
                ? obs.next(new grpc_common_1.GrpcStatusEvent(status.code, status.details, new grpc_common_1.GrpcMetadata(status.metadata)))
                : null);
    ...
    serverStream(path, req, metadata, reqclss, resclss) {
        ...
        return new rxjs_1.Observable((obs) => {
            ...
            stream.on('status', (status) => obs.next(new grpc_common_1.GrpcStatusEvent(status.code, status.details, new grpc_common_1.GrpcMetadata(status.metadata))));
            stream.on('error', (error) => {
                obs.next(new grpc_common_1.GrpcStatusEvent(error.code, error.message, new grpc_common_1.GrpcMetadata(error.metadata)));
                obs.complete();
            });
            stream.on('data', (data) => obs.next(new grpc_common_1.GrpcDataEvent(data)));
            stream.on('end', () => obs.complete());
            return () => stream.cancel();
        });
    }
